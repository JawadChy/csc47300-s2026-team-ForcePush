/**
 * HW5 TypeScript
 * This file must be compiled to JavaScript for the browser to execute.
 */

type SortKey = 'index' | 'opponent' | 'length' | 'accuracy' | 'result'
...
function parseMatchLength(text: string): number {
...
function parseResult(text: string): 0 | 1 {
...
function compareValues(
  key: SortKey,
  aRow: HTMLTableRowElement,
  bRow: HTMLTableRowElement,
): number {
...
function setAriaSort(th: HTMLTableCellElement, key: SortKey, dir: SortDir) {
  // clear other headers first, then set the active one.
  const ths = Array.from(th.closest('tr')?.querySelectorAll<HTMLTableCellElement>('th[data-sort-key]') ?? [])
...
function initMatchHistorySorting() {
  const table = document.querySelector<HTMLTableElement>('table.leaderboard-table')
  const theadRow = table?.tHead?.rows?.[0]
  const tbody = table?.tBodies?.[0]
  if (!table || !theadRow || !tbody) return

  const headerCells = Array.from(theadRow.querySelectorAll<HTMLTableCellElement>('th[data-sort-key]'))
  const state: { key: SortKey; dir: SortDir } = { key: 'index', dir: 'asc' }

  const sortRows = (nextKey: SortKey, nextDir: SortDir) => {
    ...
    for (const item of withOriginalIndex) tbody.appendChild(item.row)
    state.key = nextKey
    state.dir = nextDir

    const activeTh = headerCells.find((th) => th.dataset.sortKey === nextKey)
    if (activeTh) setAriaSort(activeTh, nextKey, nextDir)
  }

  const toggleDir = (key: SortKey): SortDir => {
    if (state.key !== key) return 'asc'
    return state.dir === 'asc' ? 'desc' : 'asc'
  }

  const getKeyFromTh = (th: HTMLTableCellElement): SortKey | null => {
